...
ClassImp(RooGenericMultiVarGaussian)
  ;

//_____________________________________________________________________________
RooGenericMultiVarGaussian::RooGenericMultiVarGaussian(const char *name, const char *title,
					 const RooArgList& xvec, const RooArgList& mu, const RooArgList& mvals) :
  RooAbsPdf(name,title),
  _x("x","Observables",this,kTRUE,kFALSE),
  _mu("mu","Offset vector",this,kTRUE,kFALSE),
  _mvals("mvals","list of cov matrix functions", this, kTRUE, kFALSE),
  _cov(mu.getSize()),
  _covI(mu.getSize()),
  _z(4)
{
...
//_____________________________________________________________________________
RooGenericMultiVarGaussian::RooGenericMultiVarGaussian(const RooGenericMultiVarGaussian& other, const char* name) : 
  RooAbsPdf(other,name), _x("x",this,other._x), _mu("mu",this,other._mu), _mvals("mvals",this,other._mvals), 
  _cov(other._mu.getSize()), _covI(other._mu.getSize()), _det(other._det), _z(other._z)
{
}
...
//_____________________________________________________________________________
void RooGenericMultiVarGaussian::syncMuVec() const 
{
...
void RooGenericMultiVarGaussian::syncMatrix() const {
  for (int i = 0; i < _mu.getSize(); ++i) {
    for (int j = 0; j < _mu.getSize(); ++j) {
      _cov(i, j) = ((RooAbsReal*)_mvals.at(i*_mu.getSize() + j))->getVal();
      _covI(i, j) = _cov(i, j);
    }
  }
  _det = _cov.Determinant();
...
//_____________________________________________________________________________
Double_t RooGenericMultiVarGaussian::evaluate() const
{
...
//_____________________________________________________________________________
Int_t RooGenericMultiVarGaussian::getAnalyticalIntegral(RooArgSet& allVarsIn, RooArgSet& analVars, const char* rangeName) const 
{
  syncMatrix();
  ...
  if (nx>127) {
    ...
    coutW(Integration) << "RooGenericMultiVarGaussian::getAnalyticalIntegral(" << GetName() << ") WARNING: p.d.f. has " << _x.getSize() 
  ...
  for (int i=0 ; i<_x.getSize() ; i++) {
	...
	cxcoutD(Integration) << "RooGenericMultiVarGaussian::getAnalyticalIntegral(" << GetName() 
			     << ") Advertising analytical integral over " << xi->GetName() << " as range is >" << _z << " sigma" << endl ;
	bits.setBit(i) ;
	anyBits = kTRUE ;
	analVars.add(*allVars.find(_x.at(i)->GetName())) ;
      } else {
	cxcoutD(Integration) << "RooGenericMultiVarGaussian::getAnalyticalIntegral(" << GetName() << ") Range of " << xi->GetName() << " is <" 
	...
	cxcoutD(Integration) << "RooGenericMultiVarGaussian::getAnalyticalIntegral(" << GetName() << ") Range of " << pi->GetName() << " is <" 
			     << _z << " sigma, relying on numeric integral" << endl ;	
      }
    }
...
//_____________________________________________________________________________
Double_t RooGenericMultiVarGaussian::analyticalIntegral(Int_t code, const char* /*rangeName*/) const 
{
  syncMatrix();
...
//_____________________________________________________________________________
RooGenericMultiVarGaussian::AnaIntData& RooGenericMultiVarGaussian::anaIntData(Int_t code) const 
{
...
//_____________________________________________________________________________
Int_t RooGenericMultiVarGaussian::getGenerator(const RooArgSet& directVars, RooArgSet &generateVars, Bool_t /*staticInitOK*/) const
{
  ...
  if (nx>127) {
    // Warn that analytical integration is only provided for the first 127 observables
    coutW(Integration) << "RooGenericMultiVarGaussian::getGenerator(" << GetName() << ") WARNING: p.d.f. has " << _x.getSize() 
...
//_____________________________________________________________________________
void RooGenericMultiVarGaussian::initGenerator(Int_t /*code*/)
{
...
//_____________________________________________________________________________
void RooGenericMultiVarGaussian::generateEvent(Int_t code)
{
...
//_____________________________________________________________________________
RooGenericMultiVarGaussian::GenData& RooGenericMultiVarGaussian::genData(Int_t code) const 
{
...
//_____________________________________________________________________________
void RooGenericMultiVarGaussian::decodeCode(Int_t code, vector<int>& map1, vector<int>& map2) const
{
  ...
  if (code<0 || code> (Int_t)_aicMap.size()) {
    cout << "RooGenericMultiVarGaussian::decodeCode(" << GetName() << ") ERROR don't have bit pattern for code " << code << endl ;
    throw string("RooGenericMultiVarGaussian::decodeCode() ERROR don't have bit pattern for code") ;
  }
...
//_____________________________________________________________________________
void RooGenericMultiVarGaussian::blockDecompose(const TMatrixD& input, const vector<int>& map1, const vector<int>& map2, TMatrixDSym& S11, TMatrixD& S12, TMatrixD& S21, TMatrixDSym& S22)
{
...
void RooGenericMultiVarGaussian::BitBlock::setBit(Int_t ibit) 
{
...
Bool_t RooGenericMultiVarGaussian::BitBlock::getBit(Int_t ibit) 
{
...
Bool_t RooGenericMultiVarGaussian::BitBlock::operator==(const BitBlock& other) 
{
